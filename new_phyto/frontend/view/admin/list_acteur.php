...
<?php
 ...
 ?>
    ...
    <a disabled href="?action=del_list_acteur&id=<?= $datas['id'] ?>" id="sp<?= $datas['id'] ?>" class="btn btn-danger" style="float:right;">supprimer</a>
        <?= $datas['nom'] ?>
        <br>
    ...
    <script>
        ...
        but<?= $datas['id'] ?>.onclick = () => {
           let x = confirm('Vous voulez vraiment supprimer "<?= $datas['nom'] ?>" dans les acteurs ? \n \n  Si vous le faite, cela pourrait détruire les statistiques déjà existante');
            if(x != true){
                return false; 
            }
