<?php $title = 'Importation en cargaison' ?>

<?php ob_start() ?>
<br><br>
<div id="si7">
<div class="container" style="background:white; padding:2%; text-align:left;">
  <h2> importations en bagage a soute pour l'annee <?= $annee ?> </h2>
  <br><br>
<table style="background-color: white;" class="table table-bordered table-striped table-condensed">
  ...
  <?php
     if($nb == 1){
        ?>
              <h3 class="alert alert-danger">Pas d'importation en bagage a soute effectuer en <?= $annee ?></h3>
        <?php
              }
      ?>
      ...
      <h4>Poid total = <?= $poid_t ?> Kg &nbsp; soit &nbsp; <?= $poid_t/1000 ?> tonne (t) </h4>
          ...
          <br><br>
          <button onClick="imprimer('si7')" class="btn btn-success">Imprimer</button>
