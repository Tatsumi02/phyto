...
<div class="container" style="background:white; padding:2%; text-align:left;">
    <h2>Importations pour <?= $product ?></h2>
    <br><br>
      ...
      <?php
        ...
        $poid_tone = 0;
            while($EXP = $importsx -> fetch()){
                $produitx = $getter->getProduitBy($EXP['id']); 
                $importations = $getter->search_prod_import($EXP['id']);
            while($exp2 = $importations->fetch()){

                $importateurs = $getter->getActeurBy($exp2['importateur_id']);
  ...
  <?php
     if($nb == 1){
        ?>
              <h3 class="alert alert-danger">Pas d'importation effectuer pour le produit <?= $product ?></h3>
        <?php
              }
      ?>
